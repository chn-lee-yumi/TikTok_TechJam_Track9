...
def preprocess_data(json_file, divisions_file):
    ...
    divisions = load_json(divisions_file)
    ...
    for post_id, post_data in data.items():
        ...
        processed_data[post_id] = {
            ...
            'target_group': target,  # not consider two or more target groups
            'label': label,
            'rationales': rationales_softmax
        }

    # 将处理后的数据按 test, train, val 分别保存为 pickle 文件
    for division in ['test', 'train', 'val']:
        division_data = {post_id: processed_data[post_id] for post_id in divisions[division]}
        with open(f'{division}_data.pkl', 'wb') as f:
            pickle.dump(division_data, f)
